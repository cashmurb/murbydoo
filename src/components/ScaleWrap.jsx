...
export default function ScaleWrap({ children, variant = "fixed" }) {
  ...
  useEffect(() => {
    if (isMobile) return;
    if (variant === "fixed") {
      const update = () => setScale(Math.min(window.innerWidth / 1440, window.innerHeight / 1024));
      update();
  ...
  if (isMobile) {
    return (
      <div style={{ width: "100%", minHeight: "100vh", background: "#FFFFFF", fontFamily: "'Kode Mono', monospace", color: "#000000", overflowX: "hidden" }}>
        {children}
      </div>
    );
  }
  ...
  if (variant === "fluid") {
    return (
      <div style={{ width: "100%", overflow: "hidden", position: "relative", display: "flex", justifyContent: "center", height: contentHeight * scale }}>
        <div ref={innerRef} style={{ width: 1440, minHeight: 1024, background: "#FFFFFF", position: "relative", fontFamily: "'Kode Mono', monospace", color: "#000000", paddingBottom: 120, flexShrink: 0, transformOrigin: "top left", transform: `scale(${scale})` }}>
          {children}
        </div>
      </div>
    );
