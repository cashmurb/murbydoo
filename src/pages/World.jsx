...
export default function World() {
  ...
  useEffect(() => {
    supabase.from('projects').select('*').order('slot').then(({ data }) => {
      ...
      data.forEach(p => {
        if (grouped[p.category]) grouped[p.category].push(p.title ? p : null);
      });
      setProjects(grouped);
    });
  }, []);
  ...
  return (
    <ScaleWrap variant="fixed">
      ...
      <div style={{ position: "absolute", left: 270, top: 283, width: 899, height: 459, border: "1px solid #000000", borderRadius: 15 }}>
        <div onClick={prevProject} style={{ position: "absolute", left: 56, top: 222, cursor: "pointer", fontSize: 24, color: "#000000", userSelect: "none" }}>&#10094;</div>
        <div onClick={nextProject} style={{ position: "absolute", right: 64, top: 222, cursor: "pointer", fontSize: 24, color: "#000000", userSelect: "none" }}>&#10095;</div>

        {currentProject ? (
          <div style={{ position: "absolute", inset: 0, left: 120, right: 120, textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: CONTENT_GAP }}>
            <div style={{ fontSize: 32, fontWeight: 600, lineHeight: 1.25, whiteSpace: "pre-line" }}>{currentProject.title}</div>
            <div style={{ fontSize: 15, color: MUTED, maxWidth: 520 }}>{currentProject.subtitle}</div>
            <div style={{ fontSize: 15, color: ACCENT }}>{currentProject.tag}</div>
            {currentProject.github && (
              <a href={currentProject.github} target="_blank" rel="noopener" style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ position: "absolute", left: 1191, top: 456, display: "flex", flexDirection: "column", gap: 38 }}>
        {CATEGORY_NAMES.map((name) => (
          <div key={name} onClick={() => setCategoryAndReset(name)} style={{ cursor: "pointer", fontSize: 15, color: category === name ? ACCENT : "#000000" }}>
            {name}
          </div>
        ))}
      </div>
