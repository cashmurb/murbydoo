...
function ArticleCard({ article, onDelete, onSave }) {
  const [form, setForm] = useState({ title: article.title, subtitle: article.subtitle || '', url: article.url || '' });
  ...
  return (
    <div style={S.card}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <div><label style={S.label}>Substack URL</label><input style={S.input} value={form.url} onChange={e => set('url', e.target.value)} /></div>
...
function AddArticle({ onAdd }) {
  ...
  const add = async () => {
    ...
    setStatus(null);
    const { data: existing } = await supabase.from('articles').select('sort_order').order('sort_order', { ascending: false }).limit(1);
    const sort_order = existing?.length ? existing[0].sort_order + 1 : 0;
    const { error } = await supabase.from('articles').insert({ ...form, sort_order });
  ...
  if (!open) return <button style={{ ...S.btn, marginBottom: 20 }} onClick={() => setOpen(true)}>+ Add article</button>;

  return (
    <div style={{ ...S.addCard, marginBottom: 20 }}>
      <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 14, color: ACCENT }}>New article</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <div><label style={S.label}>Substack URL</label><input style={S.input} value={form.url} onChange={e => set('url', e.target.value)} placeholder="https://open.substack.com/..." /></div>
        <div style={{ display: 'flex', gap: 10 }}>
...
export default function ArticlesTab() {
  ...
  return (
    <div>
      <h2 style={{ fontSize: 22, marginBottom: 8, fontWeight: 600 }}>Dump / Writing</h2>
      <p style={{ fontSize: 13, color: '#B4B4B4', marginBottom: 24 }}>These appear in the Dump carousel. Order = sort order in DB.</p>
      <AddArticle onAdd={load} />
      {articles.map(a => <ArticleCard key={a.id} article={a} onDelete={load} onSave={load} />)}
    </div>
  );
}
