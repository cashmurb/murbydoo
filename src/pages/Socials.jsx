...
function Envelope({ social }) {
  ...
  return (
    <div style={{ width: 190, display: "flex", flexDirection: "column", alignItems: "center" }}>
      <div onClick={toggle} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={{ position: "relative", width: 190, height: 188, perspective: 600, cursor: "pointer" }}>
        <div style={{ position: "absolute", inset: 0, border: "1px solid #000000", borderRadius: 2, boxShadow: glow, transition: "box-shadow .3s ease", overflow: "hidden" }}>
          ...
          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", opacity: open ? 1 : 0, transition: "opacity .35s ease .1s" }}>
            <a href={social.url} target="_blank" rel="noopener" onClick={stop} style={{ fontSize: 15, color: "#000000", textDecoration: "none", textAlign: "center", padding: "0 12px" }}>
              {social.username}
            </a>
          </div>
        </div>
